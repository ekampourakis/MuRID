...
Public Class Pixels
    Public Shared Function ResizeImage(ByVal Image As Bitmap, ByVal Scale As Integer)
        Dim Temp As New FastPixel(New Bitmap(Image.Size.Width * Scale, Image.Size.Height * Scale))
        Temp.Lock()
        Dim Original As New FastPixel(Image)
        Original.Lock()
            ...
            For y As Integer = 0 To Image.Size.Height - 1
                For h As Integer = y * Scale To y * Scale + Scale - 1
                    For w As Integer = x * Scale To x * Scale + Scale - 1
                        Temp.SetPixel(w, h, Original.GetPixel(x, y))
                    Next
                Next
            Next
        Next
        Temp.Unlock(True)
        Original.Unlock(True)
        Return Temp.Bitmap
    End Function
